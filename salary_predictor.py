...
def requiredinfo():
  ...
  quartersalary = 0
  increase_rate = [0,0,0,0,0]


  if area != 'X':
      url = 'http://api.census.gov/data/timeseries/qwi/sa?get=EarnS&for=state:' + statenumbers[states.index(area)] + '&year=2012&quarter=1&agegrp=' + agecode + '&key=12d01c707d06d007959c6906293ee268eff111d2'
      response = urllib2.urlopen(url)
      data = response.read()
      text = data.decode()
      basesalary = int(text[48:52]) * 12
  return age,agecode,area, basesalary

age,agecode,area, basesalary = requiredinfo()
